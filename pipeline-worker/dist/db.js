...
async function updateContractStatus(contractId, status, clauseCount, overallRiskScore, threadId) {
    const query = `
    UPDATE contracts
    SET status = $1, clause_count = $2, overall_risk_score = $3, thread_id = $4, updated_at = NOW()
    WHERE id = $5
  `;
    await exports.pool.query(query, [status, clauseCount, overallRiskScore, threadId, contractId]);
}
async function insertReview(review) {
    ...
    RETURNING id
  `;
    const res = await exports.pool.query(query, [
        id,
        review.contractId,
...
async function writeAuditEvent(contractId, eventType, userId, reviewId, agentName, payload) {
    const id = crypto_1.default.randomUUID();
    const query = `
