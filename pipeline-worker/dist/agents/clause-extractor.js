...
exports.ClauseExtractor = exports.CLAUSE_TYPES = void 0;
const base_agent_1 = require("./base-agent");
exports.CLAUSE_TYPES = [
    'indemnification',
...
class ClauseExtractor extends base_agent_1.BaseAgent {
    ...
    async execute(state) {
        ...
        for (const chunk of chunks) {
            const prompt = EXTRACTION_PROMPT.replace('{types}', exports.CLAUSE_TYPES.join(', ')).replace('{contract_text}', chunk);
            const raw = await this.callLlmJson(prompt);
            for (const item of raw) {
                const headingNorm = (item.heading || '').trim().toLowerCase();
                if (headingNorm && seenHeadings[headingNorm] !== undefined) {
                    console.warn(`[duplicate_clause_heading_detected] Heading: ${item.heading}, First seen ID: ${seenHeadings[headingNorm]}`);
                }
                const clause = {
                    id: nextId,
        ...
        if (expectedHeadingCount > 0 && extractionGap > 0) {
            ...
            console.error(`[clause_extraction_integrity_check_failed] Expected: ${expectedHeadingCount}, Extracted: ${allClauses.length}, Gap: ${extractionGap}`);
        }
    ...
    preSegment(text, maxChars = 12000) {
        // Split on headings
...
exports.ClauseExtractor = ClauseExtractor;
