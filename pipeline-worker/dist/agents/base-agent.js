...
const CACHE_FILE = path_1.default.join(__dirname, '../../../evals/.llm_cache_node.json');
...
class BaseAgent {
    ...
    groqProvider;
    constructor() {
        this.model = config_1.config.groqModel;
        this.groqProvider = (0, groq_1.createGroq)({ apiKey: config_1.config.groqApiKey });
    }
    systemPrompt() {
        return ("You are a senior legal AI assistant specialised in contract risk analysis. " +
    ...
    async callLlm(userPrompt, systemOverride, temperature = 0.0, maxTokens) {
        ...
        const promptHash = crypto_1.default.createHash('sha256').update(promptInput).digest('hex');
    ...
    async callLlmJson(userPrompt, maxRetries = 2, options) {
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            try {
                ...
                const startBracketIdx = Math.min(text.indexOf('{') === -1 ? Infinity : text.indexOf('{'), text.indexOf('[') === -1 ? Infinity : text.indexOf('['));
